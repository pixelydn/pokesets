...
Examples:
    python generate_g8bf.py factory-sets.json ./out
        → generates ALL tiers found in the JSON

    python generate_g8bf.py factory-sets.json ./out OU UU
        → generates only the specified tiers

Output structure:
    <output_dir>/
        styles.css              (copied from same dir as this script)
        gen8/battle-factory/
            <Tier>/<slug>/index.html

Notes:
    - styles.css must sit next to this script to be copied. If not found it is
      skipped with a warning.
    - HTML files reference ../../../../styles.css (4 levels up).
    - Uses body class "g8bf" — add a matching scope to styles.css if you want
      gen8-specific theming, or reuse .g7bf styles as-is.
"""
...
def fmt_evs(evs, level, happiness):
    """Build the EV display string."""
    parts = []
    if level:
...
def fmt_ivs(ivs):
    """Build the IV display string, or None if no non-standard IVs."""
    if not ivs:
        return None
...
def sprite_id(species):
    ...
    Uses gen5 static sprites from Pokémon Showdown.
    """
...
def make_html(species, sets, tier):
    ...
    for s in sets:
        ...
        ev_str = fmt_evs(s.get('evs', {}), s.get('level'), s.get('happiness'))
        iv_str = fmt_ivs(s.get('ivs'))
        ...
        stat_rows = (
            ...
            '<span class="row-value">{}</span></div>'
            '<div class="row"><span class="row-label">EVs</span>'
            '<span class="row-value">{}</span></div>'
        ).format(
        ...
        if iv_str:
            stat_rows += (
                '<div class="row"><span class="row-label">IVs</span>'
                '<span class="row-value">{}</span></div>'
            ).format(esc(iv_str))
    ...
    return '\n'.join([
        ...
        '    <img class="sprite" src="{}" alt="{}" onerror="this.setAttribute(\'data-broken\',1)">'.format(sprite_url, esc(species)),
...
def main():
    ...
    # Copy styles.css if available next to this script
